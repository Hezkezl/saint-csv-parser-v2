...
/**
 * php bin/console app:parse:csv GE:QuestNpcInvolved
 */

class QuestNpcInvolved implements ParseInterface
{
    use CsvParseTrait;
...
{npcmap}{endnpc}{{-stop-}}";
    ...
    public function parse()
    {
        ...
        // grab CSV files
        $questCsv = $this->csv('Questcutdown');
        ...
        //$MappyCSV = $this->csv('xivapi_mappy');
        $TerritoryTypeCSV = $this->csv('TerritoryType');
        $mapcsv = $this->csv('Map');
        $levelmodcsv = $this->csv('LevelMod');
...
//           $InstanceContent3 = $InstanceContentCsv->at($quest['InstanceContent[2]'])['Name'];
            ...
            $questend = ucwords(strtolower($ENpcResidentCsv->at($quest['Target{End}'])['Singular']));
            $questgiverid = $quest['Issuer{Start}'];
            $questendid = $quest['Target{End}'];
            ...
            if ($questendid > 0) {
                if (!empty($ENpcResidentCsv->at($quest['Target{End}'])['Singular'])) {
                                    ...
                                    $endNpcName = ucwords(strtolower($ENpcResidentCsv->at($quest['Target{End}'])['Singular']));
                                    $endNpcNumber = $quest['Target{End}'];
                                    if (empty($endNpcName)) {
                                        continue;
                                    }

                                    //grab data from level
                                    $level = $levelmodcsv->at($quest['Target{End}']);

                                    $NpcMap = $mapcsv->at($level["Map"]);
                                            if ($mapcsv->at($level["Map"])["PlaceName{Sub}"] > 0) {
                                                $NpcPlaceName = $PlaceNameCsv->at($mapcsv->at($level["Map"])["PlaceName{Sub}"])['Name'];
                                                } elseif ($mapcsv->at($level["Map"])["PlaceName"] > 0) {
                                                $NpcPlaceName = $PlaceNameCsv->at($mapcsv->at($level["Map"])["PlaceName"])['Name'];
                                                } elseif ($mapcsv->at($level["Map"])["PlaceName{Region}"] > 0) {
                                                $NpcPlaceName = $PlaceNameCsv->at($mapcsv->at($level["Map"])["PlaceName{Region}"])['Name'];
                                                } elseif ($mapcsv->at($level["Map"])["PlaceName{Region}"] < 1) {
                                                $NpcPlaceName = "Blank";
                                                }
                                    ...
                                    $offsety = $mapcsv->at($level["Map"])['Offset{Y}'];
                                    ...
                                    "{{QuestNPC|Name=". $endNpcName ."|Loc=". $NpcPlaceName ."|Coordinates=". round($NpcLocX, 1) ."-". round($NpcLocY, 1)."|ID=". $endNpcNumber ."|Quest=". $quest['Name'] ."|Questend=True}}\n";

                                    $EndNpcString[] = $string;
                                }
                            }

                $EndNpcString = implode($EndNpcString);
            ...
            if (!empty($quest['Id'])) {
                ...
                foreach(range(0,49) as $i) {
                    if (!empty($quest["Script{Instruction}[$i]"])) {
                        ...
                        //Look up all of the NPCS Involved (Actor[0-31]) and output their locations.
                        foreach(range(0,31) as $key) {
                            if ($quest["Script{Instruction}[$i]"] === "ACTOR{$key}") {
                                if (!empty($ENpcResidentCsv->at($quest["Script{Arg}[$i]"])['Singular'])) {
                                    //Retrieve the X, Z, and Map values using the Level info
                                    $NpcName = ucwords(strtolower($ENpcResidentCsv->at($quest["Script{Arg}[$i]"])['Singular']));
                                    $NpcNumber = $quest["Script{Arg}[$i]"];
                                    if (empty($NpcName)) {
                                        continue;
                                    }

                                    //grab data from level
                                    $level = $levelmodcsv->at($quest["Script{Arg}[$i]"]);
                                    ...
                                    $string = 
                                    //"\n{{QuestNPC|Name={$NpcMapName}|}}";
                                    "{{QuestNPC|Name=". $NpcName ."|Loc=". $NpcPlaceName ."|Coordinates=". round($NpcLocX, 1) ."-". round($NpcLocY, 1)."|ID=". $NpcNumber ."|Quest=". $quest['Name'] ."}}\n";

                                    $NpcsMappyInvolved[] = $string;
                                }
                            }
                        }
                    }
                ...
                $NpcsMappyInvolved = implode($NpcsMappyInvolved);
            }
            //---------------------------------------------------------------------------------
...
//                '{trait}' => $TraitRewardName,
        ...
        $this->io->text('Saving ...');
        $info = $this->save('QuestNpcInvolved.txt', 999999);
