    ...
    public function parse()
    {
      include (dirname(__DIR__) . '/Paths.php');
        ...
        foreach ($ENpcBaseCsv->data as $id => $ENpcBase) {
            ...
            if (empty($ENpcResidentCsv->at($id)['Singular'])) continue;
            //$CustomTalkArray = [];
            //$TopicSelectArray = [];
            $ENpcShopsArray = [];
            $ShopOutputData = false;
            ...
            foreach(range(0,31) as $i) {
                ...
                if(!empty($ENpcBase["ENpcData[$i]"])) {
                    /**
                     * 3538944 -> 3539050 goes to prehandler
                     * 3407872 -> 3407879 goes to LotteryExchangeShop
                     * 2752512 -> 2752515 goes to FC Shop
                     */
                    if ($ENpcBase["ENpcData[$i]"] >= 3276800 && $ENpcBase["ENpcData[$i]"] < 3279999) { //TOPIC SELECT
                        ...
                        foreach(range(0,9) as $a) {
                            if (($ENpcBase["ENpcData[$i]"] == 0) || ($TopicSelectCsv->at($ENpcBase["ENpcData[$i]"])["Shop[$a]"] == 0)) continue;
                            $ShopLink = $TopicSelectCsv->at($ENpcBase["ENpcData[$i]"])["Shop[$a]"];
                                ...
                                if ($ShopID >= 1769000 && $ShopID < 1779999) {
                                    ...
                                    $SpecialShopItemArray = [];
                                        foreach(range(0,59) as $b) {
                                            if (!empty($ItemCsv->at($SpecialShopCsv->at($ShopID)["Item{Receive}[$b][0]"])["Name"])) {
                            ...
                            if ($ShopLink >= 1769000 && $ShopLink < 1779999) { // links to SpecialShop
                                ...
                                $SpecialShopItemArray = [];
                                    foreach(range(0,59) as $b) {
                                    if (!empty($ItemCsv->at($SpecialShopCsv->at($ShopLink)["Item{Receive}[$b][0]"])["Name"])) {
                    ...
                    if ($ENpcBase["ENpcData[$i]"] >= 720000 && $ENpcBase["ENpcData[$i]"] < 722999) { //CUSTOMTALK
                        $ShopLink = $ENpcBase["ENpcData[$i]"];
                        $CustomTalkShopItemArray = [];
                        foreach(range(0,29) as $b) {
                            //cut the crap
                            if (strpos($CustomTalkCsv->at($ShopLink)["Script{Instruction}[$b]"], "SHOP")) {
                                print_r("yes");
                                if (empty($CustomTalkCsv->at($ShopLink)["Script{Instruction}[$b]"])) continue;
                                $ShopName = $CustomTalkCsv->at($ShopLink)["Name"];
                                
                                $CustomtalkArgument = $CustomTalkCsv->at($ShopLink)["Script{Arg}[$b]"];

                                if ($CustomtalkArgument >= 2752500 && $CustomtalkArgument < 2753000) { //FC SHOP
                                    $FCShopName = $FccShopCsv->at($CustomtalkArgument)["Name"];
                                    $FCShopArray = [];
                                    foreach(range(0,9) as $b) {
                                        $Item = $ItemCsv->at($FccShopCsv->at($CustomtalkArgument)["Item[$b]"])['Name'];
                                        if (!empty($Item)) {
                                            $CreditsCost = $FccShopCsv->at($CustomtalkArgument)["Cost[$b]"];
                                            $RankRequired = $FccShopCsv->at($CustomtalkArgument)["FCRank{Required}[$b]"];
                                            $FCShopArray[] = "{{Sells|Item1=" . $Item . "|Count1=" . $CreditsCost . "|Required=FC Rank " . $RankRequired . "}}";
                                        }
                                    }
                                    $ItemOutput = implode("\n", $FCShopArray);
                                ...
                                $CustomTalkShopItemArray[] = "". $CustomtalkArgument ."";
                            }
                        }
                        if (empty($CustomTalkShopItemArray)) continue;
                        $CustomTalkShopItemOutput = implode("\n", $CustomTalkShopItemArray);
                        $ShopOutput = "\n|". $ShopName ."\n". $CustomTalkShopItemOutput ."";
                        $ShopOutputData = "\n". $ShopName ."= CUSTOMTALK \n{{Tabsells\n". $ShopOutput ."}}\n\n|-|";
                    } // end of CUSTOMTALK
            ...
            $ENpcShopsOutput = "NPC NAME = ". $ENpcResidentCsv->at($id)['Singular'] . " | ". $id ."". $ENpcShopsOutput ."}}";
